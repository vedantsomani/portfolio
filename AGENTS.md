...
- Cloudflare Workers via `@astrojs/cloudflare` (static assets + Worker). Every page is prerendered except `/contact` and the action endpoint. Config in `wrangler.jsonc`; images optimised at build (`imageService: 'compile'`), no Images or KV bindings. Worker name `portfolio` (must match Workers Builds). `workers_dev` and `preview_urls` are on for now; **set `"workers_dev": false` at the Phase 4 launch** once the custom domain serves the site. (Vercel was dropped: Hobby forbids commercial use.)
